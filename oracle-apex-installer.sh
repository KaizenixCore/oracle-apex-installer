...
# ═══════════════════════════════════════════════════════════════════════════════
# HELPER FUNCTION: SAVE PASSWORDS (CRITICAL FIX for v2.4.1)
# ═══════════════════════════════════════════════════════════════════════════════
save_passwords() {
    log_info "Saving password files..."
    mkdir -p "$PROJECT_DIR"
    ...
    log_success "Password files saved securely"
}
...
step_01_init() {
    ...
    # Save passwords immediately (CRITICAL FIX)
    save_passwords
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 04: CLEANUP (FIXED - Preserves password files)
# ═══════════════════════════════════════════════════════════════════════════════
step_04_cleanup() {
    ...
    # CRITICAL FIX: Do NOT delete password files!
    # Only delete specific directories, not .db_password and .apex_password
    ...
    # CRITICAL FIX: Re-save passwords after cleanup
    save_passwords
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 18: FIX POOL CONFIGURATION (CRITICAL FOR ERROR 574) - FIXED in v2.4.1
# ═══════════════════════════════════════════════════════════════════════════════
step_18_fix_pool_config() {
    ...
    # CRITICAL FIX: Ensure password files exist before this step
...
EOF" 2>&1
    ...
    # Create pool.xml with correct password - FIXED: Use actual password value
...
step_21_verify_config() {
    ...
    # CRITICAL FIX: Verify password files exist
    [ -f "$PROJECT_DIR/.db_password" ] && log_success ".db_password exists" || { log_warning ".db_password missing - recreating"; save_passwords; }
    [ -f "$PROJECT_DIR/.apex_password" ] && log_success ".apex_password exists" || { log_warning ".apex_password missing - recreating"; save_passwords; }
...
if [ -z "$PASS" ]; then
    ...
    echo "Please run the installer again or create the file manually:"
    echo "  echo 'yourpassword' > ~/oracle-apex-complete/.db_password"
    exit 1
fi
...
مشکل اصلی را پیدا کردم! در **Step 18** شما `pool.xml` را با رمز عبور **plain text** می‌سازید، اما در **fix.sh** از `${PASS}` استفاده می‌کنید که در heredoc با single quotes (`'FIXEOF'`) **expand نمی‌شود**!

## 🐛 مشکلات شناسایی شده:

1. **مشکل اصلی:** در `fix.sh` و `pool.xml` داخل heredoc با single quotes، متغیر `${PASS}` expand نمی‌شود
2. **خطای 574:** رمز عبور در `pool.xml` باید **رمزگذاری شده** باشد (ORDS 25.x این را نیاز دارد)
3. **فایل رمز عبور:** در Step 4 (cleanup) فایل‌های `.db_password` پاک می‌شوند اما دوباره ساخته نمی‌شوند

## ✅ راه‌حل کامل

کد کامل و اصلاح شده را در چند بخش می‌دهم. **بخش اول:**
...
#  Version: 2.4.1 - GUI Crash COMPLETELY FIXED + System Restart Safe
################################################################################
